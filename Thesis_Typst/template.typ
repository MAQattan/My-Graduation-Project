...
#let setup_thesis(body) = {
  ...
  set page(
    paper: "a4",
    margin: (inside: 3.5cm, outside: 2cm, top: 2cm, bottom: 2.2cm), 
    
// الترقيم واسم الفصل في الهيدر/الفوتر مع كشف ديناميكي للنمط النشط
      footer: context {
      let page_num = counter(page).get().first()
      
      // 1. جلب نمط الترقيم النشط للصفحة الحالية (مثل "i" أو "1")
      let fmt = here().page-numbering()
      
      // 2. تنسيق رقم الصفحة بناءً على النمط
      ...
      if fmt == none {
        ...
        // جلب آخر عنوان رئيسي ظهر قبل هذه الصفحة مباشرة
        let all_levels = query(heading.where(level: 1).before(here()))
        
        // إذا كنا داخل فصل غير مرقم (كالملاحق أو المراجع)، يظهر رقم الصفحة فقط ممركزاً وينقطع تنسيق الفصول
        if all_levels.len() > 0 and all_levels.last().numbering == none {
          align(center)[#page_str]
        } else {
          // الفصول المرقمة الاعتيادية يطبق عليها التنسيق الجانبي المعتاد
          let chapters = all_levels.filter(it => it.numbering != none)
      ...
      align(center)[
        #context {
          if it.element.numbering != none {
            ...
            text(size: 30pt, weight: "bold", fill: rgb("#1E3A8A"))[#str(ch_num)]
            v(-1.6em)
          }
        }
        #text(size: 16pt, weight: "bold", fill: rgb("#1E3A8A"))[#it.element.body] 
        #v(0.6em)
